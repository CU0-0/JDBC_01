...
public class InsertRun {
	...
	public static void main(String[] args) {
		// JDBC 맛보기
		/*
		 * Java를 사용하지 않았을 때 ZONE테이블에 한 행 인서트하려면 어떤 절차가 필요했지?
		 * 
		 * 1. 컴퓨터를 켠다.
		 * 
		 * 2. 디비버 실행(클라이언트 프로그램)
		 ...
		 * 4. Oracle 선택
		 * 
		 * 5. IP주소, PORT번호, 사용자이름, 비밀번호 가지고 연결
		 * 
		 * 6. 새 SQL편집기
		 * 
		...
		// 사용자가 입력한 값을 DBMS로 보내서 => ZONE테이블에 INSERT
		...
		try {
			...
			e.printStackTrace();
			System.out.println("65행으로 가시오"); 
			System.out.println("1. ojdbc.jar 파일을 추가하지 않음");
			System.out.println("2. oracle.jdbc.driver.OracleDriver 오타확인");
		}catch (SQLException e) {
				e.printStackTrace();
				System.out.println("URL을 잘못적은 경우 -> 69행 오타 확인");
				System.out.println("username/password; logon denied -> 70행, 71행 오타확인");
