...
public class KeeperDao {
	 ...
	 * DB에 직접 접근해서 SQL문을 실행 후 결과 받아오기
	 */
	/*
	 * JDBC용 객체
	 ...
	 * 
	 * - PerparedStatement : SQL문을 미리 준비하는 개념
	 * 						미완성된 SQL문을 미리 전달하고
	 * 						실행하기전 값을 Bind해서 실행함
	 ...
	 * - 차이점
	 * 
	 * 1) Statement는 완성된 SQL문, PreparedStatment는 미완성 SQL문
	 * 
	 * 2) 객체 생성
	 ...
	 * 
	 * ? == placeholder에 실제값을 Binding해준 뒤 실행한다.
	 * pstmt.setString()
	 * pstmt.setInt()
	 */
	static{
	 ...
	 * PreparedStatement는 SQL쿼리를 최초 1회만 파싱하고 실행계획을 캐싱
	 * 재사용적인 측면에서 훨씬 효율적
	 * 
	 * 2. DB서버에 대한 트래픽 감소
	 * 
	 ...
	 * PreparedStatement는 SQL 인젝션 방지가 됨 => 보안적인 측면에서도 좋음
	 * 
	 * 
	 */
